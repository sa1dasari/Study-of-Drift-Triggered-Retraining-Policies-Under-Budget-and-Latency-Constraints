...
written to separate directories so they never overwrite the incremental
(partial_fit) results.
...
Usage:
    python main_no_partial_fit.py                            # all 4 policies, 10 seeds
    python main_no_partial_fit.py --policy periodic          # periodic only
    python main_no_partial_fit.py --policy error_threshold   # error-threshold only
    python main_no_partial_fit.py --policy drift_triggered   # drift-triggered only
    python main_no_partial_fit.py --policy no_retrain        # baseline only
    python main_no_partial_fit.py --seeds 3                  # 3-seed set
"""
...
from src.data.synthetic_data_drift_generator import DriftGenerator
...
def run_policy_sweep(policy_type, seeds):
    """Execute the full-factorial sweep for a single policy (no partial_fit)."""
    ...
    summary_csv = str(PROJECT_ROOT / f"{RESULTS_BASE}/csv/summary_results_{policy_type}_retrain_{seed_label}.csv")
        ...
        for budget in BUDGETS:
            for retrain_latency, deploy_latency in LATENCY_CONFIGS:
                for seed in seeds:
                    ...
                    # 3. Run experiment (NO partial_fit)
    ...
    plot_output = str(PROJECT_ROOT / f"{RESULTS_BASE}/plots/summary_results_plot_{policy_type}_retrain_{seed_label}.png")
    Path(plot_output).parent.mkdir(parents=True, exist_ok=True)
    plot_summary_for_policy(
        csv_path=summary_csv,
        output_path=plot_output,
        policy_name=f"{display} (No Partial Fit)",
    )
...
def _run_no_retrain_sweep(seeds):
    ...
    summary_csv = str(PROJECT_ROOT / f"{RESULTS_BASE}/csv/summary_results_{policy_type}_{seed_label}.csv")
    Path(summary_csv).parent.mkdir(parents=True, exist_ok=True)
    Path(summary_csv).unlink(missing_ok=True)
    ...
    plot_output = str(PROJECT_ROOT / f"{RESULTS_BASE}/plots/summary_results_plot_{policy_type}_{seed_label}.png")
    Path(plot_output).parent.mkdir(parents=True, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run retraining-policy experiments WITHOUT partial_fit "
                    "(static model between retrains)."
    )
    parser.add_argument(
        "--policy",
