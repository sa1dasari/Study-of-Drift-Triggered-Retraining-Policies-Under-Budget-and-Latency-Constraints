...
Usage:
    python luflow_main_no_partial_fit.py                            # all 4 policies (252 runs)
    python luflow_main_no_partial_fit.py --policy periodic          # periodic only  (81 runs)
    python luflow_main_no_partial_fit.py --policy error_threshold   # error-threshold only (81 runs)
    python luflow_main_no_partial_fit.py --policy drift_triggered   # drift-triggered only (81 runs)
    python luflow_main_no_partial_fit.py --policy no_retrain        # baseline only  (9 runs)
"""
...
# =====================================================================
#  DATA LOADING  (identical to luflow_main.py)
# =====================================================================
...
# =====================================================================
#  STREAM CONSTRUCTION  (identical to luflow_main.py)
# =====================================================================
...
def run_policy_sweep(policy_type, per_day, pool_cache):
    ...
    results_dir = PROJECT_ROOT / f"{RESULTS_BASE}/per_run/luflow_{policy_type}_{seed_label}"
    results_dir.mkdir(parents=True, exist_ok=True)

    summary_csv = str(PROJECT_ROOT / f"{RESULTS_BASE}/csv/luflow_summary_{policy_type}_retrain_{seed_label}.csv")
        ...
        for drift_type in DRIFT_TYPES:
            ...
            for budget in BUDGETS:
                for retrain_latency, deploy_latency in LATENCY_CONFIGS:
                    ...
                    # Run (NO partial_fit)
                    runner = ExperimentRunnerNoPartialFit(model, policy, metrics)
                    runner.run(X, y)
    ...
    elapsed = time.time() - start_time
    print(f"\n[NO PARTIAL FIT] LUFlow {display}: {total_runs} runs completed in "
          f"{elapsed / 60:.1f} minutes")
    print(f"Summary CSV -> {summary_csv}")

    print(f"Generating LUFlow {display} summary plot (no partial_fit) ...")
    from plot_summary import plot_summary_for_policy
    plot_output = str(PROJECT_ROOT / f"{RESULTS_BASE}/plots/luflow_summary_plot_{policy_type}_retrain_{seed_label}.png")
    Path(plot_output).parent.mkdir(parents=True, exist_ok=True)
    plot_summary_for_policy(
        csv_path=summary_csv,
        output_path=plot_output,
        policy_name=f"LUFlow {display} (No Partial Fit)",
    )
...
def _run_no_retrain_sweep(per_day, pool_cache):
    ...
    summary_csv = str(PROJECT_ROOT / f"{RESULTS_BASE}/csv/luflow_summary_{policy_type}_{seed_label}.csv")
    Path(summary_csv).parent.mkdir(parents=True, exist_ok=True)
    Path(summary_csv).unlink(missing_ok=True)
    ...
    plot_output = str(PROJECT_ROOT / f"{RESULTS_BASE}/plots/luflow_summary_plot_{policy_type}_{seed_label}.png")
    Path(plot_output).parent.mkdir(parents=True, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run LUFlow retraining-policy experiments WITHOUT "
